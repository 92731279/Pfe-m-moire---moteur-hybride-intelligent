...
def evaluate_golden(csv_path: str):
    if not os.path.exists(csv_path):
        print(colored(f"[!] Erreur: Fichier Golden Dataset {csv_path} introuvable.", "red"))
        print("Veuillez creer un CSV avec colonnes: RAW, TRUTH_NAME, TRUTH_COUNTRY, TRUTH_TOWN, TRUTH_PSTCD")
        return

    df = pd.read_csv(csv_path, sep=";")
    print(colored(f"[+] Golden Dataset charge: {len(df)} cas de test annotees.", "green"))
    ...
    total = len(summary)
    print("\n" + "="*50)
    print(colored("Rapport Qualite et Evaluation - A/B Testing", "yellow", attrs=["bold"]))
    print("="*50)
    ...
    print("\n" + colored("Accuracy par Champ (Regles  ->  Hybride)", "cyan"))
